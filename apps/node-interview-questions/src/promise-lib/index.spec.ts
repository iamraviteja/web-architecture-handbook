...
const MyPromise = PromisePolyfill
...
describe("then", () => {
  ...
  it("with chaining", () => {
    return promise({ value: 3 })
      .then((v:string | number | undefined) => (v as number) * 4)
...
describe("catch", () => {
  ...
  it("with chaining", () => {
    return promise({ value: 3 })
      .then((v:string | number | undefined) => {
        throw (v as number) * 4
      })
...
describe("finally", () => {
  it("with no chaining", () => {
    const checkFunc:any = v => v => expect(v).toBeUndefined()
  ...
  it("with multiple finallys for same promise", () => {
    const checkFunc:any = v => expect(v).toBeUndefined()
    const mainPromise = promise()
...
function promise({ value = DEFAULT_VALUE, fail = false }:PromiseParam = {}): Promise<string|number|undefined> {
